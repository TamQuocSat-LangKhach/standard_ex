...
skill:addEffect(fk.CardUsing, {
  ...
  can_trigger = function(self, event, target, player, data)
    return target == player and player:hasSkill(skill.name) and data.card.type == Card.TypeTrick and
  ...
  on_use = function(self, event, target, player, data)
    ...
    room:askToSkillInvoke(player, { skill_name = skill.name, prompt = "#jizhi-invoke:::"..card:toLogString() }) then
      room:throwCard(card.id, skill.name, player, player)
